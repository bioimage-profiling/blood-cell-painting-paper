...
from utils_harmony import one_hot_tensor, get_batch_codes
...
def correction_fast(X, R, Phi, O, ridge_lambda, device_type):
    ...
    n_clusters = R.shape[1]
    n_batches = Phi.shape[1]
    Phi_1 = torch.cat((torch.ones(n_cells, 1, device=device_type), Phi), dim=1).t()
    ...
    for k in range(n_clusters):
        ...
        Phi_t_diag_R = Phi_1 * R[:, k].view(1, -1)
        
        W = torch.matmul(torch.matmul(P_t_B_inv, P), torch.matmul(Phi_t_diag_R, X))
        W[0, :] = 0
